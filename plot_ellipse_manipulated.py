# -*- coding: utf-8 -*-
"""
Created on Tue Apr 28 10:37:41 2020
...
def Main():
    ...
    points.append(point_first)
    print(points)
    
    df = pandas.DataFrame(data=points)
    df.to_csv("./plot_ellipse.csv", sep=',',index=False)
    
    areamap = folium.Map(location=[22.3506, 114.1849])    
    folium.PolyLine(points).add_to(areamap)
    areamap.save('plot_ellipse.html')
...
def GetEllipsePoints(p1_lat, p1_lng, p2_lat, p2_lng, perimeter_points_in_meters):
    ...
    print('Center coordinates = ', center_lat, center_lng)
